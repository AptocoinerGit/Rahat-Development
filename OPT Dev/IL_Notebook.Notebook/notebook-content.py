...
from pyspark.sql.functions import col, desc, when, lit, coalesce
...
if Drop_table_if_mismatch:
    ...
    for filename in os.listdir(folder_path_json):
        ...
        try:
            existing_cols = {}
            for col_info in spark.catalog.listColumns(f"{table_name}"):
...
for entry in os.scandir(folder_path):
    ...
    table_exists = target_table in [t.name for t in spark.catalog.listTables()]
    ...
    if table_exists:
        ...
        join_expr = ' AND '.join([f"source.`{k}` = target.`{k}`" for k in key_cols])
        update_set = {c: f"source.{c}" for c in column_names}
